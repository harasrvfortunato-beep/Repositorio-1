/**
 * LEADUP — interactive 3D logo (Three.js).
 * Builds a metallic "LEAD" + electric-blue "UP" wordmark with an
 * integrated growth arrow, and exposes a small viewer class that
 * both the intro screen and the hero stage instantiate.
 */
...
const FONT_URL = new URL('../assets/fonts/helvetiker_bold.typeface.json', import.meta.url).href;
...
function buildArrow(size, material) {
  const group = new THREE.Group();
  const shaftLen = size * 0.62;
  const shaft = new THREE.Mesh(
    new THREE.CylinderGeometry(size * 0.05, size * 0.05, shaftLen, 8),
    material
  );
  ...
  const head = new THREE.Mesh(
    new THREE.ConeGeometry(size * 0.14, size * 0.34, 10),
    material
  );
  head.rotation.z = -Math.PI / 2;
  head.position.x = shaftLen + size * 0.1;
  group.add(head);
...
/**
 * Assembles the full LEADUP wordmark as a centered THREE.Group.
 */
async function buildLogoGroup() {
  ...
  const lead = buildTextMesh(font, 'LEAD', silverMat, size);
  const leadWidth = lead.geometry.boundingBox.max.x - lead.geometry.boundingBox.min.x;

  const up = buildTextMesh(font, 'UP', blueMat, size);
  up.position.x = leadWidth + size * 0.08;

  const upWidth = up.geometry.boundingBox.max.x - up.geometry.boundingBox.min.x;
  const upHeight = up.geometry.boundingBox.max.y - up.geometry.boundingBox.min.y;

  const arrow = buildArrow(size * 0.9, blueMat);
  arrow.position.set(
    up.position.x + upWidth - size * 0.08,
    upHeight * 0.62,
    size * 0.08
  );
 ...
 * pointer/touch drag-to-rotate (Y axis) and inertia. Call `.enable3D()`
 * to kick off loading — cheap to construct, only allocates a GL
 * context once the font/mesh promise resolves.
 */
